...
class PostsDailyList extends PureComponent {
  ...
  getDateRange(after, before) {
    ...
    const range = _.range(daysCount).map(i =>
      moment(before, 'YYYY-MM-DD')
        .subtract(i, 'days')
        .startOf('day')
    )
    return range
  ...
  loadMoreDaysInc(e) {
    ...
    const loadMoreAfter = moment(this.state.after, 'YYYY-MM-DD')
      .subtract(numberOfDays, 'days')
      .format('YYYY-MM-DD')
    const loadMoreBefore = moment(this.state.after, 'YYYY-MM-DD')
      .subtract(1, 'days')
      .format('YYYY-MM-DD')
    ...
    return (
      <div className="posts-daily">
        ...
        {this.state.loading ? (
          ...
          <a
            className="posts-load-more posts-load-more-days"
            onClick={this.loadMoreDays}
          >
...
registerComponent('PostsDailyList', PostsDailyList, withCurrentUser, [
  withList,
  options
])
