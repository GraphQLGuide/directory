...
Categories.getParents = function(category, store) {
  ...
  const getParents = function recurse(category) {
    if (category && category.parentId) {
      const parent = store
        ? Categories.findOneInStore(store, category.parentId)
        : Categories.findOne(category.parentId)
      if (parent) {
...
Categories.getChildren = function(category) {
  ...
  var getChildren = function recurse(categories) {
    var children = Categories.find({
      parentId: { $in: _.pluck(categories, '_id') }
    }).fetch()
...
Posts.getCategories = function(post) {
  return !!post.categories
    ? Categories.find({ _id: { $in: post.categories } }).fetch()
    : []
}
/**
