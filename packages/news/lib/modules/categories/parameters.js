...
function PostsCategoryParameter(parameters, terms, apolloClient) {
  ...
  if (cat && cat.length) {
    ...
    const categoriesIds = _.pluck(
      _.filter(allCategories, category =>
        _.contains(categoriesSlugs, category.slug)
      ),
      '_id'
    )

    const operator =
      getSetting('forum.categoriesFilter', 'union') === 'union' ? '$in' : '$all'
