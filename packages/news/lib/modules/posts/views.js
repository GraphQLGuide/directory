...
Posts.addView('userUpvotedPosts', (terms, apolloClient) => {
  var user = apolloClient
    ? Users.findOneInStore(apolloClient.store, terms.userId)
    : Users.findOne(terms.userId)
...
Posts.addView('newsletter', terms => {
  ...
  const lastNewsletterIsAfterLastWeek =
    lastNewsletter && moment(lastNewsletter.createdAt).isAfter(lastWeek)
  const after = lastNewsletterIsAfterLastWeek
    ? lastNewsletter.createdAt
    : lastWeek.toDate()
