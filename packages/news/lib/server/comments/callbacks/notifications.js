...
function CommentsNewNotifications(comment) {
  ...
  if (Meteor.isServer && !comment.disableNotifications) {
    ...
    if (
      Users.getSetting(postAuthor, 'notifications_comments', false) &&
      comment.userId !== postAuthor._id
    ) {
    ...
    if (!!comment.parentCommentId) {
      ...
      if (
        ...
        if (
          Users.getSetting(parentCommentAuthor, 'notifications_replies', false)
        ) {
          createNotification(parentComment.userId, 'newReply', {
            documentId: parentComment._id
          })
