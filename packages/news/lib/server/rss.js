...
export const servePostRSS = (terms, url) => {
  ...
  postsCursor.forEach(post => {
    ...
    const feedItem = {
      title: post.title,
      description:
        description + `<a href="${Posts.getPageUrl(post, true)}">Discuss</a>`,
      author: post.author,
      date: post.postedAt,
      guid: post._id,
      url:
        getSetting('forum.RSSLinksPointTo', 'link') === 'link'
          ? Posts.getLink(post)
...
export const serveCommentRSS = (terms, url) => {
  ...
  const commentsCursor = Comments.find(
    { isDeleted: { $ne: true } },
  ...
  commentsCursor.forEach(function(comment) {
    ...
    feed.item({
      ...
      description: `${comment.body}</br></br><a href='${Comments.getPageUrl(
        comment,
        true
      )}'>Discuss</a>`,
...
Picker.route('/rss/category/:slug/feed.xml', function(params, req, res, next) {
  res.end(
    servePostRSS(
      { view: 'new', cat: params.slug },
      '/rss/category/:slug/feed.xml'
    )
  )
})
