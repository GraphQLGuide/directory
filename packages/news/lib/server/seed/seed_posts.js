...
registerSetting(
  'forum.seedOnStart',
  true,
  'Seed the app with dummy content on startup'
)
...
if (getSetting('forum.seedOnStart')) {
  ...
  const createPost = async (slug, postedAt, username, thumbnail) => {
    const currentUser = await Users.rawCollection().findOne({
      username: username
    })
    const document = {
      postedAt: postedAt,
    ...
    if (typeof thumbnail !== 'undefined') {
      document.thumbnailUrl =
        '/packages/example-forum/lib/assets/images/' + thumbnail
    }
  ...
  const createComment = async (slug, username, body, parentBody) => {
    ...
    const parentComment = await Comments.rawCollection().findOne({
      body: parentBody
    })
  ...
  const createDummyPosts = async () => {
    ...
    return Promise.all([
      ...
      createPost(
        'removing_getting_started_posts',
        moment()
          .subtract(2, 'days')
          .toDate(),
        'Julia'
  ...
  const createDummyComments = async () => {
    ...
    return Promise.all([
      ...
      createComment(
        'deploying',
        'Julia',
        'Do you really need to say this all the time?',
        'Deploy to da choppah!'
      ),
